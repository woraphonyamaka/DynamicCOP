...
normtvtpx=function(theta,data,z,rhobar,Call="optim"){
...
for (jj in  2:T){
   if (jj<=10){
      psi1[jj] = w+ a*kappa[jj-1] + b*(mean(abs(x[1:jj-1]*y[1:jj-1])))+d*z[jj]
   }else{
      psi1[jj] = w+ (a*kappa[jj-1]) + b*(mean(abs(x[(jj-10):(jj-1)]*y[(jj-10):(jj-1)])))+d*z[jj]
   }
...
if (is.nan(CL))  # control for optimization
    CL<--n*100
cat("Sum of log Likelihood for normdynamicX ->",sprintf("%4.4f",c(CL,kappa[n])),"\n")
...
##---------------
dynamicnormalx=function(data, z, plot){
lower =rep(-5,4)
upper =rep(5,4)
theta=c(w=0.01,beta1=0.002,beta2=-0.002,exo=0.1)


model <- optim(theta,normtvtpx,data=data,rhobar=0.5, z=z,Call="optim",
...
rhot=normtvtpx(coef,data=data,rhobar=0.5,z=z,Call="filtering")$TVTP
if ( plot==TRUE){
  plot(ts( rhot[5:n]))}

...
Ttvtp=function(theta,data,Call="optim"){
...
for (jj in  2:n){
 if (jj<=10){
      psi1[jj,1] = w+ a*kappa[jj-1,1] + b*(mean(abs(x[1:jj-1]*y[1:jj-1])))
...
kappa[jj,2] = (exp( psi1[jj,2])/(1+exp(psi1[jj,2])))*98 + 2.01   # a modified logistic transformation df
 ...
 psi1[jj,1] = w+  a*kappa[jj-1,1]  + b* (mean(abs(x[(jj-10):(jj-1)]*y[(jj-10):(jj-1)])))
...
if (is.nan(CL))  # control for optimization
    CL<--n*100
cat("Sum of log Likelihood for Tdynamic ->",sprintf("%4.4f",c(CL,kappa[n])),"\n")
...
model <- optim(theta,Ttvtp,data=data, Call="optim",
...
rhot=Ttvtp(coef,data=data,Call="filtering")$TVTP
if ( plot==TRUE){
  plot(ts( rhot[5:n]))}
